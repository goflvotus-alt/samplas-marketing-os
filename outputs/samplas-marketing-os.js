...
function normalizedRouteHash(hash) {
  return hash === "todaySalesCalendar" || /^monthly-report-ch\d+$/.test(hash) ? "monthly-report" : hash;
}
...
function setActiveView(view, options = {}) {
  ...
  $("#monthlyCalendarSlot")?.toggleAttribute("hidden", routeHash === "annual-report");
...
function monthlyReportRankRows(items, options = {}) {
  ...
  // MONTHLY-INTEL-NAV: labelHtmlFn은 optional 확장 지점이다 — 넘기지 않으면 기존과 동일하게
  // labelFn 결과를 esc()해서 <strong> 안에 그대로 넣는다(하위 호환, 기존 호출부 전부 무변경).
  // 넘기면 raw HTML을 그대로 쓴다(호출부가 직접 escape 책임 — monthlyIntelLink가 이미 처리).
  const labelHtmlFn = options.labelHtmlFn || null;
  ...
  return rows.map((item, index) => {
    ...
    return `<div class="monthly-report-rank-row"${attrs ? ` ${attrs}` : ""}>
      ...
      <div class="monthly-report-rank-main">
        <strong>${labelHtmlFn ? labelHtmlFn(item, index) : esc(labelFn(item))}</strong>
...
// MONTHLY-INTEL-NAV: Monthly에 표시되는 데이터를 관련 Intelligence 화면으로 연결하는 공용
// hover+click 링크. storeIntelJumpLink(STORE-INTEL-UI-B)와 동일한 검증된 메커니즘을 그대로
// 재사용한다 — 항상 DOM에 존재하는 절대위치 popover를 hover/focus 시 opacity로만 보여주므로
// layout shift가 절대 발생하지 않는다. jumpAttr는 호출부가 만든 data-* 속성 문자열이다
// (예: data-jump-view="..." — 기존 델리게이트 핸들러를 그대로 타므로 새 라우팅 코드가 없다).
function monthlyIntelLink(labelHtml, ariaLabel, jumpAttr, popoverInnerHtml) {
  return `<button type="button" class="monthly-intel-link" ${jumpAttr} aria-label="${esc(ariaLabel)}">${labelHtml}<span class="monthly-intel-popover" aria-hidden="true">${popoverInnerHtml}</span></button>`;
}

// MONTHLY-INTEL-NAV: monthlyIntelLink의 inline 변형 — 기존 텍스트 흐름(예: rank-row의
// <strong> 안, 결제수단 legend의 <strong> 자리) 안에 자연스럽게 섞여야 하는 곳에 쓴다.
// wrapTag를 주면(예: "strong") 그 태그로 감싸서 반환한다 — monthlyReportRankRows처럼
// 호출부가 이미 <strong>을 갖고 있으면 wrapTag 없이 그 안에 바로 넣는다.
function monthlyIntelLinkInline(labelHtml, ariaLabel, jumpAttr, popoverInnerHtml, wrapTag) {
  const button = `<button type="button" class="monthly-intel-link-inline" ${jumpAttr} aria-label="${esc(ariaLabel)}">${labelHtml}<span class="monthly-intel-popover" aria-hidden="true">${popoverInnerHtml}</span></button>`;
...
// PART B(STORE-BATCH-D): ALL일 때 온라인/압구정/VAIL 3축 구성 계산. offlineSnapshot(merged,
// storeCode 보존)에서 매장별 합계를 더한다 — 새 API 호출 없음. MONTHLY-INTEL-NAV: 이 계산을
...
function monthlyAllStoreBreakdownNote(offlineSnapshot, archive) {
  const { byStore, storesIncluded, hasLines } = computeMonthlyStoreOfflineBreakdown(offlineSnapshot);
  if (!hasLines && !storesIncluded.length) return "";
...
// MONTHLY-INTEL-NAV: "매장 매출 비율" 도넛 — 압구정/VAIL 두 매장이 모두 분리 업로드된
// 달에만 렌더링한다(한쪽만 있으면 2-slice 도넛이 의미가 없으므로 렌더링을 건너뛴다 — 기존
// "미분류" 텍스트 fallback과 동일한 보수적 원칙). 실제 canonical store offline 금액만
// 사용하고 예시 숫자는 하드코딩하지 않는다.
function monthlyStoreDonutBlock(offlineSnapshot) {
  const { byStore, storesIncluded } = computeMonthlyStoreOfflineBreakdown(offlineSnapshot);
  if (!storesIncluded.includes("APGUJEONG") || !storesIncluded.includes("VAIL")) return "";
  const total = byStore.APGUJEONG + byStore.VAIL;
  if (total <= 0) return "";
  const rows = [
    { code: "APGUJEONG", color: "var(--yellow)", amount: byStore.APGUJEONG },
    { code: "VAIL", color: "var(--blue)", amount: byStore.VAIL }
  ].map((row) => ({
    ...row,
    share: monthlyReportRatio(row.amount, total),
    label: STORE_FILTER_LABELS[row.code] || row.code,
    viewName: row.code === "APGUJEONG" ? "ApgujeongIntelligence" : "VailIntelligence"
  }));
  // 각 조각을 독립적으로 hover/click 가능하게 하려면 conic-gradient 한 장(원 전체가 하나의
  // hit-area)이 아니라 SVG stroke-dasharray 기반 arc 2개가 필요하다 — dasharray의 "빈 구간"은
  // 기본적으로 hover/click 대상이 되지 않으므로(브라우저 표준 hit-testing), 각 <circle>이
  // 정확히 자기 몫의 원호에서만 반응한다. 실선 legend <button>이 완전한 키보드/스크린리더
  // 경로를 담당하고, arc는 마우스 hover 강조 + 클릭 이동을 보조로 제공한다(arc 자체에
  // tabindex는 주지 않는다 — 포커스만 가고 Enter로 활성화되지 않는 반쪽 접근성을 피하기 위해).
  const radius = 48;
  const circumference = 2 * Math.PI * radius;
  let cursor = 0;
  const arcs = rows.map((row) => {
    const len = (row.share / 100) * circumference;
  ...
  const legendItems = rows.map((row) => {
    ...
    return `<li>
      <button type="button" class="monthly-intel-link monthly-store-donut-legend-row" data-jump-view="${esc(row.viewName)}" aria-label="${esc(row.label)} — ${esc(pct(row.share))} · ${esc(won(row.amount))} · ${esc(row.label)} Intelligence로 이동">
        ...
        <span class="monthly-store-donut-legend-text"><strong>${esc(row.label)}</strong><span>${esc(pct(row.share))} · ${esc(won(row.amount))}</span></span>
        <span class="monthly-intel-popover" aria-hidden="true">${popover}</span>
      </button>
    </li>`;
  }).join("");
  return `<section class="monthly-report-block">
    ...
    <div class="monthly-store-donut-card">
      <div class="monthly-store-donut">
        ...
        <div class="clients-donut-center"><strong>${esc(pct(rows[0].share))}</strong><span>${esc(rows[0].label)}</span></div>
      </div>
      <ul class="clients-donut-legend monthly-store-donut-legend">${legendItems}</ul>
    </div>
  </section>`;
}
...
function monthlyStorePerformanceBlock(offlineSnapshot, previousOfflineSnapshot) {
  ...
  const cards = ["APGUJEONG", "VAIL"].map((code) => {
    ...
    return `<article class="monthly-store-performance-card">
      ...
      <div class="monthly-store-performance-kpis">
        <div><span>이번 달 매출</span><strong>${included ? apiWon(amount) : "미분류"}</strong></div>
        <div><span>오프라인 내 비중</span><strong>${share === null ? "확인 불가" : pct(share)}</strong></div>
  ...
  return `<div class="monthly-store-performance-grid">${cards}</div>`;
}
...
async function annualStoreScopeNote(year, months, storeCode) {
  ...
  const coverageNote = missingMonths.length
    ...
    : " (연중 전 개월 매장별 업로드 완료)";
  // PART C: "매장별 분석 가능 기간"은 hardcode하지 않고 실제로 업로드가 확인된 달 중
  // 가장 이른 달을 그대로 쓴다(uploadedMonths는 위에서 이미 실측으로 채워짐).
  const earliestMonth = [...uploadedMonths].sort()[0];
  return `매장 필터: ${label} — 오프라인 매출 ${won(total)} 누적${coverageNote}. 매장별 분석 가능 기간: ${esc(earliestMonth)} ~. 아래 브랜드/상품 상세는 매장 구분 없는 전체(ALL) 기준입니다. 온라인 매출은 매장별로 구분되지 않으므로 이 숫자에 합산하지 않습니다.`;
}
...
async function renderMonthlyArchiveReport(month, renderSeq) {
  ...
  // PART B: ALL이면 이미 받아온 offlineSnapshot.salesLines(merged, storeCode 보존)에서
  // 매장별 합계를 그 자리에서 더하기만 한다 — 추가 API 호출 없음.
  const storeScopeNote = storeFilterState !== "ALL"
    ? await monthlyStoreScopeNote(month, storeFilterState)
    : monthlyAllStoreBreakdownNote(offlineSnapshot, archive);
  ...
  const previousBrandByCode = new Map(previousBrandSales.map((row) => [monthlyReportBrandCode(row), row]));
  const overallBrandPerformanceBlock = performanceBrandSales.length ? `<section class="monthly-report-block">
    <div class="monthly-report-block-head"><h4>전체 브랜드 TOP 5</h4><span>canonical brand · 온라인 + 오프라인</span></div>
    <div class="monthly-report-rank">
      ${monthlyReportRankRows(performanceBrandSales.slice(0, 5), {
        ...
        labelFn: brandPerformanceDisplayName,
        subFn: (item) => {
          const previous = previousBrandByCode.get(monthlyReportBrandCode(item));
          return previous ? `전월 ${apiWon(brandPerformancePaidAmount(previous))} · ${monthlyReportDelta(brandPerformancePaidAmount(item), brandPerformancePaidAmount(previous), apiWon)}` : "전월 비교 데이터 없음";
        },
        formatValue: (value) => apiWon(value),
        labelHtmlFn: (item, index) => {
          const previous = previousBrandByCode.get(monthlyReportBrandCode(item));
          return monthlyIntelBrandLabelHtml(item, brandPerformancePaidAmount(item), previous ? brandPerformancePaidAmount(previous) : undefined, item.quantitySold, index);
        }
      })}
    </div>
  ...
  target.innerHTML = `
    ...
    <nav class="monthly-report-toc" aria-label="Monthly report chapters">
      <a href="#todaySalesCalendar">01 Daily Sales</a>
      <a href="#monthly-report-ch2">02 Sales Structure</a>
      <a href="#monthly-report-ch3">03 Store Performance</a>
      <a href="#monthly-report-ch4">04 Brand Performance</a>
      <a href="#monthly-report-ch5">05 Online Summary</a>
    </nav>

    <section id="monthly-report-ch2" class="monthly-report-chapter">
      <div class="monthly-report-chapter-head">
        <span>02</span>
        <div><p class="eyebrow">Sales Structure</p><h3>월간 매출 구조</h3></div>
      </div>
      ${salesSummaryBlock}
    </section>
    ...
    <section id="monthly-report-ch5" class="monthly-report-chapter">
      <div class="monthly-report-chapter-head">
        <span>05</span>
        <div><p class="eyebrow">Online Summary</p><h3>온라인 판매 요약</h3></div>
      </div>
      <div class="monthly-online-summary">
        <div><span>온라인 매출</span>${onlineSalesLink}</div>
        <div><span>주문수</span>${onlineOrdersLink}</div>
        <div><span>객단가</span>${onlineAovLink}</div>
      </div>
      <div class="monthly-report-drill">
        <span>결제수단 · 브랜드 · 상품 상세는 Commerce에서 확인합니다.</span>
        <button class="today-jump-button" type="button" data-jump-view="Sales">Commerce →</button>
      </div>
    </section>
  `;
}
...
function renderCommerceSecondaryKpi() {
  ...
  // PART D: 주문수/객단가는 Cafe24 canonical(온라인 전용) 지표라 물리 매장 필터 대상이
  // 아니다 — storeCode 선택 시 그 매장의 값처럼 보이지 않도록 명시적으로 표시한다.
  target.innerHTML = [
    ["주문수" + (storeCode ? " (매장 필터 적용 대상 아님)" : ""), `${apiNum(cafeTotals.orderCount)}건`],
    ["객단가" + (storeCode ? " (매장 필터 적용 대상 아님)" : ""), apiWon(cafeTotals.averageOrderValue)],
...
function todaySummarySalesInfo(totalSales = {}, cafeTotals = {}, storeCode = null) {
  ...
  const byStore = totalSales?.offlineSales?.byStore || null;
  const storesIncluded = Array.isArray(totalSales?.coverage?.storesIncluded) ? totalSales.coverage.storesIncluded : [];
  // STORE-BATCH-D: ALL/Store Focus 어느 쪽이든 필요하면 canonical ALL 분모(회사 전체
  // 매출)를 구할 수 있게 한다 — byStore는 storeCode 필터와 무관하게 항상 실제 매장별
  // 값이므로, 이 셋을 더하면 항상 진짜 ALL 총매출이다(별도 API 호출 없이 재사용).
  const canonicalAllTotal = byStore && onlineAvailable
    ? onlineSales + Number(byStore.APGUJEONG || 0) + Number(byStore.VAIL || 0)
    : null;
  ...
  if (storeCode) {
    ...
    // PART 4: Store Focus의 secondary KPI — "전체 회사 매출 대비 비중". 분모는 반드시
    // 같은 기간의 canonical ALL total이다(그 매장만의 부분합이 아님).
    const share = storeHasData && offlineAvailable && canonicalAllTotal > 0
      ? `${((offlineSales / canonicalAllTotal) * 100).toFixed(1)}%`
      : null;
    return {
      label: `오프라인 매출 (${label})`,
...
// STORE-BATCH-D: storeCode가 있으면(Store Focus Mode) 온라인은 이 화면의 대상이 아니다
// (Clients identity는 매장별로 복제하지 않지만, 이 KPI 카드들은 "그 매장에서 실제 발생한
...
function renderClientsSummaryCards(summary = {}, ff = {}, storeCode = null, storeCoverage = null) {
  ...
  if (storeCode) {
    const label = STORE_FILTER_LABELS[storeCode] || storeCode;
    const hasData = Boolean(storeCoverage?.includedMonths?.length);
    const naText = hasData ? undefined : "데이터 없음";
    const missingNote = hasData ? "" : ` · ${storeCoverage?.missingMonths?.join(", ") || "이 기간"} 매장별 미업로드`;
    target.innerHTML = [
      salesKpiCard(`${label} 고객 수`, naText || `${apiNum(summary.totalClients)}명`, `${label} 오프라인 구매 고객 기준${missingNote}`),
      salesKpiCard(`${label} 구매 건수`, naText || `${apiNum(summary.totalPurchaseCount)}건`, `오프라인만(온라인은 매장 필터 대상 아님)${missingNote}`),
      salesKpiCard(`${label} 매출`, naText || apiWon(summary.totalSalesAmount), `오프라인만(온라인은 매장 필터 대상 아님)${missingNote}`),
      salesKpiCard("평균 구매금액", naText || apiWon(summary.avgOrderValue), `${label} 매출 / ${label} 구매 건수`),
      salesKpiCard("FF · 직원 구매", naText || `${apiNum(ff.purchaseCount)}건`, naText ? "매장 필터 데이터 없음" : `${apiNum(ff.clientCount)}명 · ${apiWon(ff.salesAmount)}`)
    ].join("");
    return;
  }
  target.innerHTML = [
...
// STORE-BATCH-B: store 쿼리 없이 조회하면(기존과 동일) 매장별 스냅샷을 합친 ALL
// 뷰가 오므로, storesMissing이 있는데 storesIncluded도 있으면(한 매장만 업로드됨)
// "Partial"으로 구분해 완전성 상태를 숨기지 않는다(Part 6).
async function refreshEcountOfflineCard(month) {
  ...
  if (badge) {
    ...
    badge.innerHTML = `<i></i>${!ok ? "Error" : partial ? "Partial" : "Healthy"}`;
  }
...
// MONTHLY-INTEL-NAV: Monthly의 brandSales 행은 이미 canonical brand_code를 갖고 있다(서버
// 파이프라인이 확정한 값 — BON CO/POP CO/SUN CO 같은 raw operational 이름이 아니다). Brand
// Selector는 표시 NAME으로 keyed되어 있으므로, 정확히 같은 brand_code를 가진 항목을 찾아
// 그 표시 NAME을 반환한다(문자열 유사매칭 없음 — resolveRawBrandCanonical과 동일한 "정확히
// 일치하지 않으면 null" 원칙). 클릭 시점에 호출한다(렌더 시점에는 initBrandSelector()의
// 비동기 fetch가 아직 안 끝났을 수 있어 race condition이 생기기 때문 — 클릭은 항상 페이지
// 로드 한참 후에 일어나므로 이 시점엔 brandSelectorIdentityByName이 이미 채워져 있다).
function resolveBrandCodeToSelectorName(brandCode) {
  const normalized = String(brandCode || "").trim().toUpperCase();
  if (!normalized) return null;
  for (const [displayName, entry] of brandSelectorIdentityByName) {
    if (String(monthlyReportBrandCode(entry) || "").trim().toUpperCase() === normalized) return displayName;
  }
  return null;
}
...
// PART F(Brand Revenue/Brand Store Share): buildBrandCustomerComposition()이 이미
// storeCode 필터와 무관하게 계산해 주는 revenueByStore를 그대로 보여준다(새 계산 없음).
// 온라인 브랜드 매출은 이 오프라인 전용 엔드포인트에 없으므로 3축(온라인 포함)이 아니라
// "이 브랜드의 매장별 오프라인 매출"만 정직하게 보여준다 — 온라인 수치를 억지로
// 끼워넣지 않는다.
function renderEntityCompositionStoreNote(data = {}) {
  const target = $("#entityCompositionStoreNote");
  if (!target) return;
  if (data?.error) {
    target.textContent = "";
    return;
  }
  const revenueByStore = data.revenueByStore || {};
  const storesIncluded = Array.isArray(data.storesIncluded) ? data.storesIncluded : [];
  const parts = ["APGUJEONG", "VAIL"].map((code) => {
    const label = STORE_FILTER_LABELS[code] || code;
    return storesIncluded.includes(code) ? `${label} ${won(revenueByStore[code] || 0)}` : `${label} 미분류`;
  });
  if (data.storeCode) {
    const label = STORE_FILTER_LABELS[data.storeCode] || data.storeCode;
    const brandStoreTotal = (revenueByStore.APGUJEONG || 0) + (revenueByStore.VAIL || 0);
    const share = data.storeHasData && brandStoreTotal > 0
      ? `${(((revenueByStore[data.storeCode] || 0) / brandStoreTotal) * 100).toFixed(1)}%`
      : null;
    target.textContent = data.storeHasData
      ? `이 브랜드 오프라인 매출(${label}) — ${won(revenueByStore[data.storeCode] || 0)}${share ? ` · 이 브랜드의 매장별 오프라인 매출 중 ${label} 비중 ${share}` : ""}. 온라인은 매장 귀속 대상 아님.`
      : `이 기간에는 ${label} 분리 업로드가 없습니다(미업로드) — 이 브랜드의 매장별 매출을 확인할 수 없습니다.`;
    return;
  }
  target.textContent = `매장별 오프라인 매출: ${parts.join(" · ")}. 온라인 매출은 이 지표에 포함되지 않습니다.`;
}
...
async function refreshEntityCustomerComposition(brandCode, month) {
  ...
  renderEntityCompositionStoreNote(data);
...
async function refreshEntityScore(brandCode, periodMonth) {
  ...
  if (prevMonth) {
    ...
    // STORE-BATCH-D: Customer Momentum은 store-specific source(Customer Composition)가
    // 있으므로 Store Focus 시 store-scoped로 계산한다. Revenue/Order Momentum은 archive
    // 파이프라인(company-wide, ALL만 지원)을 재계산하지 않는다(Score formula 자체는
    // 변경하지 않는다는 지시에 따름) — 두 컴포넌트만 store-scoped인 partial 상태다.
    const scoreStoreQuery = storeFilterState !== "ALL" ? `&store=${storeFilterState}` : "";
    const [curData, prevData] = await Promise.all([
      getEntityCompositionJson(`/api/brand-intelligence/${encodeURIComponent(brandCode)}/customer-composition?month=${encodeURIComponent(periodMonth)}${scoreStoreQuery}`),
      getEntityCompositionJson(`/api/brand-intelligence/${encodeURIComponent(brandCode)}/customer-composition?month=${encodeURIComponent(prevMonth)}${scoreStoreQuery}`)
    ]);
    if (seq !== entityScoreRefreshSeq) return;
  ...
  // STORE-BATCH-D: entityInventoryItemsState는 ECOUNT 전체(company-wide) 재고다 — 아직
  // WH_CD Store Filter가 없다(STORE-BATCH-E 예정). Store Focus Mode에서 이 전체 재고값을
  // 그 매장의 값처럼 몰래 쓰지 않는다 — unavailable(null) 처리하고 기존 partial coverage
  // normalization(availableWeight>=60 규칙)이 나머지 3개 컴포넌트만으로 정규화하게 둔다.
  let inventoryPoints = null;
  if (storeFilterState === "ALL" && entityInventoryItemsState.ready && entityInventoryItemsState.brandCode === brandCode && !entityInventoryItemsState.fetchFailed) {
...
async function refreshEntityClientsOverview(month) {
  ...
  const { monthStart, monthEnd } = monthlyReportMonthRange(month);
  // STORE-BATCH-D: Brand Intelligence의 Customer Workspace/Recent Orders도 공유
  // storeFilterState를 그대로 쓴다 — Clients 화면과 동일한 buildClientsOverview()
  // storeCode 필터를 재사용하므로 새 로직 없음.
  const url = intelligenceUrl(`/api/intelligence/clients?since=${monthStart}&until=${monthEnd}${storeFilterState !== "ALL" ? `&store=${storeFilterState}` : ""}`);
...
function bind() {
  ...
  $("#storeFilterSelect")?.addEventListener("change", (event) => {
    ...
    if ($("#Clients")?.classList.contains("active")) refreshClientsView();
    // STORE-BATCH-D: Brand Intelligence(BrandDashboard)도 공유 storeFilterState를 쓴다.
    // 전체 브랜드/기간 파이프라인(refreshEntityTrendMonths)을 다시 돌리지 않고, store-aware로
    // 만든 3개(Customer Composition/Clients Overview/Score)만 다시 부른다.
    if ($("#BrandDashboard")?.classList.contains("active") && brandIdentityState.brandCode) {
      const periodMonth = currentEntityPeriodMonthKey();
      refreshEntityCustomerComposition(brandIdentityState.brandCode, periodMonth);
      refreshEntityClientsOverview(periodMonth);
      refreshEntityScore(brandIdentityState.brandCode, periodMonth);
    }
  });
  ...
  document.addEventListener("click", (event) => {
    ...
    const monthlyTocLink = event.target.closest('.monthly-report-toc a[href^="#monthly-report-ch"], .monthly-report-toc a[href="#todaySalesCalendar"]');
    if (monthlyTocLink) {
      event.preventDefault();
    ...
    // MONTHLY-INTEL-NAV: 브랜드 데이터 → Brand Intelligence 이동은 view 전환 하나로 끝나지
    // 않는다(해당 브랜드까지 선택해야 함) — 그래서 data-jump-view 대신 별도 attribute를 쓴다.
    // brand_code → Brand Selector 표시 NAME 해석은 클릭 시점에 한다(위 주석 참고 — race
    // condition 회피). 해석에 실패하면(Brand Master에 없는 극히 드문 경우) 조용히 아무 것도
    ...
    // 필요한 경우 — data-inventory-intel-open-registry가 이미 쓰는 것과 같은 패턴(뷰 전환 후
    // setTimeout으로 scrollIntoView)이다.
    const scrollLink = event.target.closest("[data-monthly-intel-scroll-view]");
    if (scrollLink) {
      const viewName = scrollLink.dataset.monthlyIntelScrollView;
      const targetId = scrollLink.dataset.monthlyIntelScrollTarget;
      document.querySelector(`[data-view="${viewName}"]`)?.click();
      if (targetId) {
        setTimeout(() => document.getElementById(targetId)?.scrollIntoView({ behavior: "auto", block: "start" }), 200);
      }
      return;
    }
